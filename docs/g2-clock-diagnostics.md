...
不健康快照的本地错误会保留监控器发布的受限原因码，例如
`Clock snapshot is unhealthy: lease_expired`。因此真实 Actor 审计若因 PTP
报告停止、映射过期或其他门控失败，终端和 `audit/summary.json` 会直接显示具体
原因，不再只有笼统的 `Clock snapshot is unhealthy`；这只改善诊断，不放宽门控。
