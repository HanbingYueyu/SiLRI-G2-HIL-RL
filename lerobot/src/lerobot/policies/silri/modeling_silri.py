...
class SiLRIPolicy(
    ...
    def get_optimizer_and_scheduler(self):
        groups = {key: list(values) for key, values in self.get_optim_params().items()}
        if hasattr(self, 'discrete_actor'):
            groups['actor'].extend(p for name, p in self.discrete_actor.named_parameters()
                                   if not name.startswith('encoder'))
        groups = {key: list({id(p): p for p in values if p.requires_grad}.values())
                  for key, values in groups.items()}
        optim_dict = {
            "actor": torch.optim.Adam(groups['actor'], lr=self.config.actor_lr),
            "critic": torch.optim.Adam(groups['critic'], lr=self.config.critic_lr),
            "expert": torch.optim.Adam(groups['expert'], lr=self.config.actor_lr),
            "lagrange": torch.optim.Adam(groups['lagrange'], lr=0.01 * self.config.critic_lr),
        }
    ...
    def compute_loss_expert(self, observations, actions, observation_features: Tensor | None = None, is_intervention: Tensor | None = None) -> Tensor:
        ...
        loss_expert = loss_expert.sum() / is_intervention.sum().clamp_min(1)
    ...
    def compute_loss_actor_bc(self, observations, actions, observation_features: Tensor | None = None, is_intervention: Tensor | None = None) -> Tensor:
        ...
        loss_actor_bc = loss_actor_bc.sum() / is_intervention.sum().clamp_min(1)
        return {
    ...
    def compute_loss_lagrange(self, observations, observation_features: Tensor | None = None) -> Tensor:
        with torch.no_grad():
            ...
            mean_distance = (actions_expert - actions_model).pow(2).sum(-1)
    ...
    def compute_loss_discrete_actor(
        ...
        mask = is_intervention.reshape(-1) == 1
        if not torch.any(mask):
            # Keep a zero loss connected to the actor graph so backward remains valid.
            return {
                "loss_actor": actions_pi.sum() * 0.0
            }
    ...
    def compute_loss_actor(
        ...
        combine_BC = (expert_actions - model_actions).pow(2).sum(dim=-1)
    ...
    def _init_critics(self, continuous_action_dim):
        ...
        self.critic_target = CriticEnsemble(
            encoder=copy.deepcopy(self.encoder_critic), ensemble=target_heads, output_normalization=self.normalize_targets
        )
    ...
    def _init_actor(self, continuous_action_dim):
        ...
        self.actor_target = Policy(
            encoder=copy.deepcopy(self.encoder_actor),
            network=MLP(input_dim=self.encoder_actor.output_dim, **asdict(self.config.actor_network_kwargs)),
            action_dim=continuous_action_dim,
            encoder_is_shared=self.shared_encoder,
            fixed_std=torch.tensor([5e-2]),
...
class Policy(nn.Module):
    def __init__(
        ...
        self.register_buffer('fixed_std', fixed_std, persistent=False)
